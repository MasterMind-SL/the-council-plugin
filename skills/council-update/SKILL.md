...
2. **Preserve all existing data**: Do NOT delete or modify existing decisions, lessons, active memory, or archives. All v3.0.0 memory is fully compatible with v3.1.0.

3. **Update plugin_version**: Set `"plugin_version": "3.1.0"` in `index.json` and save.

4. **Report what's new in v3.1.0**:
   ...
   - **`/council:build` pipeline**: 3 consultations (PRD, tech deck, backlog) + feature completeness gate + implementation team (3-4 members with subagent parallelization).
   - **Feature completeness gate**: Verifies 100% of requested features are assigned to a workstream before implementation starts.
   ...
   - **Unified banned words**: All 5 agents share the same complete banned words list (scope creep, P0/P1/P2, defer, out of scope, fast-follow, future phase, descope, weeks, months, sprint).
   - **Synonym expansion**: `extract_topics()` expands synonyms (e.g. "autoscaling" → "performance", "postgres" → "database") and bigrams across 50+ pairs.
   ...
   - **3-tier packing**: `build_memory_response()` adapts detail to budget (generous/normal/tight).
   - **Archive top-12 by relevance**: Archive excerpts select the most relevant lessons (was last-5 by recency), with scan and token caps.
   - **MEMORY LENS directives**: Each teammate receives a role-specific lens before the injected memory block.

### From v3.1.0-beta to v3.1.0

The schema bumped from v1→v2 with a new `last_validated` field on memory entries.
...
2. **Verify schema version**: Read each `*-active.json` file in `.council/memory/`. Each should have `"version": 2`. If any still show `"version": 1`, they will auto-migrate on next council operation.

3. **Update plugin_version**: Set `"plugin_version": "3.1.0"` in `index.json` and save.

4. **No data loss**: All existing entries are fully compatible. The `last_validated` field defaults to `created` for legacy entries when computing staleness.

5. **Report what's new**: Same feature list as "From v3.0.0 to v3.1.0" above.

### From v3.1.0 to v3.1.0 (already current)

Report: "Already on v3.1.0. No migration needed." and stop.
